...
class NaSchSimulationModel:
    def __init__(self, seed: int) -> None:
        ...
        self._edge_cells: Dict[EdgeId, List[int]] = {}
    ...
    def reset(self, topology: TopologyData, config: SimulationConfig) -> SimulationState:
        ...
        self._config = config
        self._edge_cells = {
            edge_id: [0] * max(1, edge_data.n_cells)
    ...
    def step(self) -> tuple[SimulationState, SimulationMetrics, bool]:
        ...
        for vehicle_id in vehicle_ids:
            ...
            new_velocity = min(vehicle.velocity + 1, edge_data.vmax_cells)
            gap = self._gap_ahead(vehicle)
            new_velocity = min(new_velocity, gap)
            if new_velocity > 0 and self._random.random() < self._config.noise_prob:
                new_velocity -= 1
            ...
            if new_velocity > 0:
                self._edge_cells[current_edge][vehicle.cell_pos] = 0
                target_position = vehicle.cell_pos + new_velocity
                edge_length = len(self._edge_cells[current_edge])
                ...
                while target_position >= edge_length:
                    ...
                    if self._edge_cells[current_edge][clamped] == 0:
                        vehicle.cell_pos = clamped
                        self._edge_cells[current_edge][clamped] = vehicle_id
                    else:
                        previous_edge = vehicle.current_edge
                        self._edge_cells[previous_edge][vehicle.cell_pos] = vehicle_id
                        new_velocity = 0

                vehicle.distance_traveled_m += new_velocity * CELL_SIZE_M
            else:
                self._edge_cells[current_edge][vehicle.cell_pos] = vehicle_id
    ...
    def _spawn_until(self, target_count: int) -> None:
        ...
        while len(self._vehicles) < target_count and attempts < target_count * 8 + 8:
            ...
            if not route:
                continue
            start_edge = route[0]
            cells = self._edge_cells[start_edge]
            free_positions = [index for index, value in enumerate(cells[:3]) if value == 0]
            if not free_positions:
                continue
            vehicle_id = self._next_vehicle_id
            self._next_vehicle_id += 1
            start_pos = free_positions[0]
            self._vehicles[vehicle_id] = Vehicle(vid=vehicle_id, route=route, cell_pos=start_pos)
            self._edge_cells[start_edge][start_pos] = vehicle_id
    ...
    def _gap_ahead(self, vehicle: Vehicle) -> int:
        current_cells = self._edge_cells[vehicle.current_edge]
        ...
        next_cells = self._edge_cells[vehicle.next_edge]
    ...
    def _density(self) -> float:
        occupied = sum(sum(1 for value in cells if value != 0) for cells in self._edge_cells.values())
        total = sum(len(cells) for cells in self._edge_cells.values())
